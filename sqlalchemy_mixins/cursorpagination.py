...
class PageInfo:
    def __init__(self, start_cursor: str = None, end_cursor: str = None,
                 has_next_page: bool = False, has_previous_page: bool = False):
...
class PageCursor:
    def __init__(self, total_count: int = 0, edges: List[EdgeNode] = None,
                 items: List[Dict] = None, page_info: PageInfo = None):
...
class CursorPaginationMixin(SmartQueryMixin):
    ...
    async def paginate_with_cursors(cls, page_size: int = None, after_cursor: Any = None, before_cursor: Any = None,
                                    filters: Dict = None, sort_by: List[str] = None):

        total_count: int = (await cls.session.execute(cls.query)).scalar().count()
        items = None
        ...
        if page_size is None:
            ...
            return PageCursor(**{
                ...
                'page_info': cls.build_page_info(**{
                    'start_cursor': cls.encode_cursor(items[0].id) if qs else None,
                    'end_cursor': cls.encode_cursor(items[-1].id) if qs else None,
                }),
            })

        cursor_limit = {}
        if after_cursor is not None:
            cursor_limit = {'id__gt': cls.decode_cursor(after_cursor)}

        if before_cursor is not None:
            cursor_limit = {'id__lt': cls.decode_cursor(before_cursor)}
        ...
        page_info = {
            'start_cursor': cls.encode_cursor(items[0].id),
            'end_cursor': cls.encode_cursor(items[-1].id),
        }
    ...
    def build_node(cls, item: Any):
        return EdgeNode(**{
            'cursor': cls.encode_cursor(item.id),
            'node': item
        })

    @classmethod
    def build_page_info(cls, first_cursor: str = None, last_cursor: str = None,
